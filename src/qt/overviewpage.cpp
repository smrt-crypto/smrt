...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeSmrtAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSmrtAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeSmrtAmount = strAnonymizeSmrtAmount.remove(strAnonymizeSmrtAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSmrtAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSmrtAmount * COIN) nMaxToAnonymize = nAnonymizeSmrtAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSmrtAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeSmrtAmount));
        strAnonymizeSmrtAmount = strAnonymizeSmrtAmount.remove(strAnonymizeSmrtAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSmrtAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
